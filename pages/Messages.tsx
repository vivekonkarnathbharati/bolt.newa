...
const INITIAL_THREADS: Thread[] = [
  ...
  },
  {
    ...
    unread: 1,
    messages: [
      { id: 'm7', senderId: 'u3', text: 'Coffee tomorrow morning? ☕️', time: min(400) },
    ],
  },
  {
    id: 't4',
  ...
  },
  {
    ...
    unread: 0,
    messages: [
      { id: 'm10', senderId: 'u5', text: 'see you at the trailhead at 6am?', time: min(1440) },
    ],
  },
  {
    id: 't6',
    ...
    unread: 0,
    messages: [
      { id: 'm11', senderId: 'u6', text: 'the translation review is done ✅', time: min(4320) },
    ],
  },
];
...
function Avatar({ profile, size = 48, ring = false }: { profile: Profile; size?: number; ring?: boolean }) {
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <div
        className={`w-full h-full rounded-full bg-gradient-to-br ${grad} flex items-center justify-center font-bold text-white ${ring ? 'ring-2 ring-cyan-400/60 ring-offset-2 ring-offset-slate-950' : ''}`}
      ...
      {profile.online && (
        <span
          className="absolute bottom-0 right-0 rounded-full bg-emerald-400 border-2 border-slate-950"
...
export function MessagesPage() {
        ...
        prev.map((t) =>
          t.id === threadId
            ? {
                ...
                messages: [
                  ...t.messages,
                  { id: `r${Date.now()}`, senderId: profile.id, text: pick, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) },
                ],
              }
            : t
        )
  ...
  const send = () => {
    ...
    setThreads((prev) =>
      prev.map((t) => (t.id === activeId ? { ...t, messages: [...t.messages, msg], unread: 0 } : t))
    );
    setDraft('');
  ...
  const filtered = query
    ? threads.filter((t) => t.profile.name.toLowerCase().includes(query.toLowerCase()) || t.profile.handle.toLowerCase().includes(query.toLowerCase()))
    : threads;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex">
      ...
      {/* ---------------------------------------------------------- */}
      <aside
        className={`flex flex-col border-r border-white/10 bg-slate-900/40 backdrop-blur-xl
        ...
        <div className="px-5 pt-6 pb-3">
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-xl font-bold tracking-tight">
              Messages
              <span className="ml-2 text-sm font-normal text-slate-500">{threads.length}</span>
            </h1>
            <button className="p-2 rounded-full bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20 transition">
              <Icon.Plus size={20} />
            </button>
          </div>
          {/* Search */}
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl pl-9 pr-3 py-2.5 text-sm placeholder:text-slate-500 focus:outline-none focus:border-cyan-400/50 focus:ring-2 focus:ring-cyan-400/20 transition"
            />
          </div>
        </div>
          ...
          {filtered.map((t) => {
            ...
            return (
              <button
                ...
                className={`group w-full flex items-center gap-3 px-3 py-3 rounded-2xl transition-all text-left
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between gap-2 mt-0.5">
                    <p className="text-sm text-slate-400 truncate">
                      {last?.senderId === ME_ID && <span className="text-slate-500">You: </span>}
                      {last?.text}
                    </p>
                    {t.unread > 0 && (
                      <span className="shrink-0 min-w-[20px] h-5 px-1.5 rounded-full bg-cyan-400 text-slate-950 text-[11px] font-bold flex items-center justify-center">
                        {t.unread}
                      </span>
                    )}
                  </div>
      ...
      <main className={`flex-1 flex flex-col min-w-0 ${activeId ? 'flex' : 'hidden md:flex'}`}>
        ...
        <header className="flex items-center gap-3 px-5 py-4 border-b border-white/10 glass">
          <button
            onClick={() => setActiveId('')}
            className="md:hidden p-1.5 rounded-full hover:bg-white/10 transition text-slate-300"
          >
            <Icon.Back size={20} />
          </button>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-emerald-400 flex items-center gap-1.5">
              {activeThread.profile.online ? (
                <>
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                  {activeThread.profile.lastSeen}
                </>
              ) : (
                <span className="text-slate-500">last seen {activeThread.profile.lastSeen}</span>
              )}
            </p>
          </div>
          <div className="flex items-center gap-1">
            <button className="p-2.5 rounded-full hover:bg-white/10 transition text-slate-300 hover:text-cyan-400">
              <Icon.Phone />
            </button>
            <button className="p-2.5 rounded-full hover:bg-white/10 transition text-slate-300 hover:text-cyan-400">
              <Icon.Video />
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-2">
          ...
          <div className="flex items-center justify-center mb-4">
            <span className="text-[11px] uppercase tracking-wider text-slate-500 bg-white/5 px-3 py-1 rounded-full">
              Today
            </span>
          </div>
          ...
          {activeThread.messages.map((m, i) => {
            ...
            return (
              <div
                key={m.id}
                className={`flex items-end gap-2 animate-msg-in ${mine ? 'justify-end' : 'justify-start'} ${grouped ? 'mt-0.5' : 'mt-3'}`}
              >
                {!mine && (
                  <div className={`transition-opacity ${grouped ? 'opacity-0' : 'opacity-100'}`}>
                ...
                <div className={`max-w-[75%] md:max-w-[60%] flex flex-col ${mine ? 'items-end' : 'items-start'}`}>
                  <div
                    className={`px-4 py-2.5 text-[15px] leading-relaxed break-words
                      ${mine
                        ? 'bg-gradient-to-br from-cyan-400 to-indigo-500 text-slate-950 font-medium rounded-2xl rounded-br-md shadow-lg shadow-cyan-500/20'
                        : 'bg-white/8 text-slate-100 rounded-2xl rounded-bl-md border border-white/5'
                      }`}
                  >
                    {m.text}
                  </div>
                  <span className={`text-[10px] text-slate-500 mt-1 px-1 ${mine ? 'text-right' : ''}`}>
                    {m.time}
                  </span>
                </div>
              </div>
        ...
        <div className="px-4 md:px-8 py-4 border-t border-white/10 glass">
          <div className="flex items-center gap-2">
            <button className="p-2.5 rounded-full hover:bg-white/10 transition text-slate-400 hover:text-cyan-400 shrink-0">
              <Icon.Plus />
            </button>
            <div className="flex-1 flex items-center gap-2 bg-white/5 border border-white/10 rounded-full pl-4 pr-1.5 py-1.5 focus-within:border-cyan-400/50 focus-within:ring-2 focus-within:ring-cyan-400/20 transition">
              <input
                ...
                placeholder="Message…"
                className="flex-1 bg-transparent outline-none text-[15px] placeholder:text-slate-500"
              />
              <button className="p-2 rounded-full hover:bg-white/10 transition text-slate-400 hover:text-amber-400 shrink-0">
                <Icon.Smile />
              </button>
              <button className="p-2 rounded-full hover:bg-white/10 transition text-slate-400 hover:text-rose-400 shrink-0">
                <Icon.Heart />
              </button>
            </div>
            <button
              onClick={send}
              disabled={!draft.trim()}
              className="shrink-0 w-11 h-11 rounded-full bg-gradient-to-br from-cyan-400 to-indigo-500 text-slate-950 flex items-center justify-center transition-all hover:scale-105 hover:shadow-lg hover:shadow-cyan-500/30 disabled:opacity-30 disabled:scale-100 disabled:shadow-none"
      ...
      {/* Keyframes + utilities (scoped via <style> so the file is drop-in) */}
      <style>{`
        .glass {
